...
class MenuExtension_ownCloud(GObject.GObject, Nemo.MenuProvider):
    def __init__(self):
        GObject.GObject.__init__(self)
    ...
    # The get_file_items method of Nemo.MenuProvider no longer takes
    # the window argument. To keep supporting older versions of Nemo,
    ...
    def ask_for_menu_items(self, files):
        ...
        for action, enabled, label in menu_items:
            item = Nemo.MenuItem(name=action, label=label, sensitive=enabled)
    ...
    def legacy_menu_items(self, files):
        ...
        entry = socketConnect.nemoVFSFile_table.get(filename)
        if not entry:
            return []
        ...
        if state_ok:
            ...
            for key, value in socketConnect.nemoVFSFile_table.items():
        ...
        if 'COPY_PRIVATE_LINK_MENU_TITLE' in self.strings:
            item_copyprivatelink = Nemo.MenuItem(
        ...
        if 'EMAIL_PRIVATE_LINK_MENU_TITLE' in self.strings:
            item_emailprivatelink = Nemo.MenuItem(
...
class SyncStateExtension_ownCloud(GObject.GObject, Nemo.InfoProvider):
    def __init__(self):
        GObject.GObject.__init__(self)
